...
for data := range myConcurrentSeq {
    @await() {
      <div>{ data() }</div>
    }
}

@awaitFallible("<div>Failed to render this section</div>") {
    <div>{ myDataOrError() }</div>
}

@awaitFallible(fancyErrorFallback("Failed to render this section as well")) {
    <div>{ myDataOrError() }</div>
}
```
